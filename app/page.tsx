...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const qId = q.get('tg_id'), qName = q.get('name')
    if (!id && qId) id = Number(qId)
    if (qName) name = qName
    if (!id) { id = Number(String(Date.now()).slice(-9)); name = `${name} (демо)` }
    setTg({ name, id, startParam: unsafe?.start_param || q.get('start') || '' })
  }, [])
  ...
  function aHashFromCanvas(canvas: HTMLCanvasElement): string {
    ...
    const tctx=t.getContext('2d')!; tctx.drawImage(canvas,0,0,w,h)
    const {data}=tctx.getImageData(0,0,w,h); let sum=0; const g:number[]=[]
    for (let i=0;i<data.length;i+=4){ const v=0.299*data[i]+0.587*data[i+1]+0.114*data[i+2]; g.push(v); sum+=v }
    const avg=sum/g.length; let bits=''; for (const v of g) bits+= v>avg ? '1':'0'
  ...
  async function onStartVerification() {
    ...
    try {
      ...
      const snap=snapRef.current!; snap.width=w; snap.height=h
      setGoodFrames(0); setStep('scanning'); setStatus('Повільно повертайте голову та тримайте обличчя в колі')
      let best:any = null
      const loop = async () => {
        ...
        setGoodFrames(g=>{ const next = ok ? Math.min(minGoodFrames,g+1) : Math.max(0,g-2); ringRef.current?.style.setProperty('--progress', String(next/minGoodFrames)); return next })
        if (best && goodFrames>=minGoodFrames){
          const ctx=snap.getContext('2d')!; ctx.drawImage(v,0,0,w,h)
          ...
          setStatus('Перевірка…')
          const r = await fetch('/api/verify-mode', { method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ face_id: tg.id, dataUrl, ahash, descriptor, tg_user_id: tg.id, display_name: tg.name, demo: String(tg.name).includes('(демо)')?true:undefined })})
          const json = await r.json()
          if (!r.ok) setResult({ ok:false, msg: json.error || r.statusText })
          else setResult({ ok:true, passed: json.passed, dist: json.dist })
          setStep('result'); setBusy(false); return
        }
        setTimeout(loop,80)
      }
      loop()
    } catch(e:any) {
      setBusy(false); setStatus(e?.message||'Помилка доступу до камери')
    }
  }

  const wrap: React.CSSProperties = { minHeight:'100dvh', padding:'max(16px, env(safe-area-inset-top)) max(16px, env(safe-area-inset-right)) max(16px, env(safe-area-inset-bottom)) max(16px, env(safe-area-inset-left))', display:'grid', placeItems:'center', background:'#000', color:'#fff' }
  const card: React.CSSProperties = { width:'min(96vw, 520px)', background:'#111', borderRadius:20, boxShadow:'0 10px 40px rgba(0,0,0,0.5)', padding:18, border:'1px solid rgba(255,255,255,0.06)' }
  const btn: React.CSSProperties = { width:'100%', marginTop:14, padding:'16px', borderRadius:14, border:'none', background:'#0a84ff', color:'#fff', fontWeight:700, fontSize:'clamp(15px, 3.6vw, 16px)', cursor:'pointer' }
  const small: React.CSSProperties = { fontSize:'clamp(11px, 2.8vw, 12px)', opacity:.7, marginTop:10 }
    ...
    <div style={wrap}>
      {step==='intro' && (
        <section style={card}>
          ...
          <div style={{ marginTop:8, opacity:.9 }}>Вітаємо, <b>{tg.name}</b></div>
          ...
          <button onClick={onStartVerification} disabled={busy||!consent} style={{ ...btn, opacity: busy||!consent ? .6 : 1 }}>Пройти верифікацію</button>
      ...
      {step==='scanning' && (
        <section style={{ ...card, width:'min(98vw, 720px)' }}>
          ...
          <div style={{ fontSize:'clamp(11px, 2.8vw, 12px)', opacity:.7, marginTop:6 }}>Облич у кадрі: {faces}</div>
        ...
        <section style={card}>
          ...
          {result?.ok ? (
            <div style={{ marginTop:10, fontSize:'clamp(15px, 4vw, 18px)' }}>
              ...
              {'dist' in (result||{}) && (<div style={{ fontSize:'clamp(11px, 2.8vw, 12px)', opacity:.7, marginTop:6 }}>dist: {result?.dist?.toFixed?.(3)}</div>)}
            </div>
          ) : (
            <div style={{ color:'#ff4d4f', marginTop:10 }}>{'Помилка: ' + (result?.msg || 'Невідома помилка')}</div>
          )}
          <button onClick={()=>{ setStep('intro'); setResult(null); setStatus(''); setConsent(false) }} style={{ ...btn, background:'#34c759' }}>Готово</button>
        </section>
      )}
    </div>
  )
